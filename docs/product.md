...
- A CLI named `slr` for selecting models, starting or stopping the local proxy, checking daemon status, and viewing model usage counters.
...
- Local selection state, latency observations, and usage counters under `~/.sleepy-llm-router`.

## Product invariants

- The package does not auto-start a daemon during install; users explicitly run `slr start` or `slr start --daemon`.
- Only models selected by `slr model` are eligible for request routing.
...
- `slr model --all` uses the same recommendation-sorted display order; explicit `--select` keeps the user-provided order.
- `slr model --group fast|balanced|capable --select ...` assigns group pools while also keeping those models eligible in the main selected allowlist.
